  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${strings.tabTitleInstances}</div>
          <div class="card-sub">${strings.tabSubInstances}</div>
        </div>
      ...
      ? html`<div class="muted">${strings.noInstances}</div>`
...
function renderEntry(entry: PresenceEntry) {
  ...
  const lastInput =
    entry.lastInputSeconds != null
      ? strings.secondsAgo(entry.lastInputSeconds)
      : "n/a";
  ...
  const scopesLabel =
    scopes.length > 0
      ? scopes.length > 3
        ? strings.numScopes(scopes.length)
        : `${strings.scopesLabel} ${scopes.join(", ")}`
      : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.host ?? strings.unknownHost}</div>
      ...
      <div class="list-meta">
        ...
        <div class="muted">${strings.lastInput} ${lastInput}</div>
        <div class="muted">${strings.reasonLabel} ${entry.reason ?? ""}</div>
      </div>
    </div>
  `;
}
