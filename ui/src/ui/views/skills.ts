  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${strings.tabTitleSkills}</div>
          <div class="card-sub">${strings.tabSubSkills}</div>
        </div>
...
function renderSkill(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  if (skill.disabled) reasons.push(strings.disable);
  if (skill.blockedByAllowlist) reasons.push(strings.blocked);
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          ...
          ${skill.disabled ? html`<span class="chip chip-warn">${strings.disable}</span>` : nothing}
        </div>
        ${missing.length > 0
      ? html`
