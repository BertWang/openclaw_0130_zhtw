...
export function renderLogs(props: LogsProps) {
  ...
  const exportLabel = needle || levelFiltered ? strings.exportFiltered : strings.exportVisible;

  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${strings.logsTitle}</div>
          <div class="card-sub">${strings.logsSub}</div>
        </div>
        <div class="row" style="gap: 8px;">
          ...
          >
            ${strings.exportAction(exportLabel)}
          </button>
        </div>
      </div>
      ...
      props.onFilterTextChange((e.target as HTMLInputElement).value)}
            placeholder=${strings.searchLogs}
          />
        </label>
        <label class="field checkbox">
          <span>${strings.autoFollow}</span>
          <input
            type="checkbox"
      ...
      ${props.file
      ? html`<div class="muted" style="margin-top: 10px;">${strings.logFile(props.file)}</div>`
      : nothing}
      ${props.truncated
      ? html`<div class="callout" style="margin-top: 10px;">
            ${strings.logTruncated}
          </div>`
      : nothing}
      ${props.error
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${filtered.length === 0
      ? html`<div class="muted" style="padding: 12px;">${strings.noLogEntries}</div>`
      : filtered.map(
        (entry) => html`
                <div class="log-row">
