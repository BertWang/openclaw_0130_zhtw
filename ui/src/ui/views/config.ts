...
// Section definitions (dynamic localization handled in render)
const getBaseSections = (strings: any) => [
  { key: "env", label: strings.sectionEnv },
  { key: "update", label: strings.sectionUpdate },
  { key: "agents", label: strings.sectionAgents },
  { key: "auth", label: strings.sectionAuth },
  { key: "channels", label: strings.sectionChannels },
  { key: "messages", label: strings.sectionMessages },
  { key: "commands", label: strings.sectionCommands },
  { key: "hooks", label: strings.sectionHooks },
  { key: "skills", label: strings.sectionSkills },
  { key: "tools", label: strings.sectionTools },
  { key: "gateway", label: strings.sectionGateway },
  { key: "wizard", label: strings.sectionWizard },
];
...
export function renderConfig(props: ConfigProps) {
  const strings = t();
  const SECTIONS = getBaseSections(strings);
  const validity =
    props.valid == null ? "unknown" : props.valid ? strings.healthOk : strings.healthOffline;
    ...
    <div class="config-layout">
      ...
      <aside class="config-sidebar">
        <div class="config-sidebar__header">
          ...
          <span class="pill pill--sm ${props.valid ? "pill--ok" : props.valid === false ? "pill--danger" : ""}">${validity}</span>
        </div>
