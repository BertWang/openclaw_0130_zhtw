...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">${strings.publicKey}</span>
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${strings.tabSubChannels} (Nostr)</div>
      ${accountCountLabel}
      ...
      : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${strings.configured}</span>
                <span>${summaryConfigured ? strings.healthOk : strings.healthOffline}</span>
              </div>
              <div>
                <span class="label">${strings.running}</span>
                <span>${summaryRunning ? strings.healthOk : strings.healthOffline}</span>
              </div>
              <div>
                <span class="label">${strings.publicKey}</span>
      ...
      <div class="row" style="margin-top: 12px;">
        <button class="btn" @click=${() => props.onRefresh(false)}>${strings.refresh}</button>
      </div>
    </div>
  `;
}
