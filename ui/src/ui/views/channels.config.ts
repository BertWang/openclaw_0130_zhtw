...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!node) {
    return html`<div class="callout danger">${strings.channelConfigSchemaUnavailable}</div>`;
  }
...
export function renderChannelConfigSection(params: {
  ...
  return html`
    <div style="margin-top: 16px;">
      ...
      ? html`<div class="muted">${strings.loadingConfigSchema}</div>`
      ...
      <div class="row" style="margin-top: 12px;">
        ...
        >
          ${props.configSaving ? strings.saving : strings.save}
        </button>
        <button
          class="btn"
          ?disabled=${disabled}
